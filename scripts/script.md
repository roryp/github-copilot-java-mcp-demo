...
| Explorer | With the wizard closed, expand the Todo package and `templates` folder in the workspace already on screen. | "Rather than generate a second copy, I'll return to the completed sample I cloned ahead of time. It keeps the structure the Spring Initializr created and adds the Todo code and web page." |
...
| Where | Do | Say |
|-------|----|-----|
| `pom.xml` | Show the dependency **`spring-ai-starter-mcp-server-webmvc`** and the `spring-ai-bom` 2.0.0. | "I've cloned the finished Todo sample and opened it in VS Code. I already have GitHub Copilot installed and signed in. The GitHub URL for the sample is in the video description. The app already has the operations I want Copilot to use. To expose them through MCP, I'll start with Spring AI's WebMVC MCP server starter. The Spring AI Bill of Materials manages the dependency version, which is 2.0.0 here." |
...
| Where | Do | Say |
|-------|----|-----|
| VS Code — Extensions view (`Ctrl+Shift+X`) | Search **`@mcp playwright`**, select the Playwright MCP server, and choose **Install** to add it to the VS Code user profile. Review the publisher and configuration, confirm that you trust the server, and wait for it to start. Open the Chat tools picker and confirm the Playwright tools are available. | "I've cloned the finished Todo sample and opened it in VS Code. I already have GitHub Copilot installed and signed in. The GitHub URL for the sample is in the video description. To test this app through a real browser, Copilot first needs the Playwright tools. I'll search the MCP server gallery from the Extensions view and choose Install. That adds Playwright to my VS Code user profile without changing this workspace's MCP configuration. After I review and trust the server, VS Code starts it, discovers its tools, and makes them available to Copilot Chat." |
...
| Where | Do | Say |
|-------|----|-----|
| `templates/index.html` | Show that the working page has a form and Todo list but no linked stylesheet. | "I've cloned the finished Todo sample and opened it in VS Code. The GitHub URL for the sample is in the video description. The app works, but this template has no stylesheet, so the page still uses the browser's default design. I'll delegate that small visual change and leave the Java code alone." |
| Copilot Chat — New Chat Session | Start a blank Chat with **New Chat** (`Ctrl+N`). At the lower left of the composer, open the **Local** destination picker and choose **Cloud** under **Continue In**. In **Select repository**, choose the current demo repository. Enter: *"Polish the Todo page with a small stylesheet. Add `src/main/resources/static/styles.css` and link it from the Thymeleaf template. Use a centered responsive layout, readable typography, styled text input, Add and Delete buttons, and Todo rows. Make completed Todos muted with struck-through text. Preserve all routes, behavior, accessibility labels, and `data-testid` attributes. Do not add JavaScript or frontend dependencies. Open a pull request."* Submit the task. | "I'll start a new Chat session, open the Local destination picker, and choose Cloud. Then I'll select this repository so the coding agent has the right project. My prompt names the visual goal, the files and controls to style, and the boundaries it must preserve. The agent can now work asynchronously and open a pull request." |
| Copilot Chat — completed cloud session | After the agent finishes, return to the cloud session, review its summary, and open the draft pull request from the provided link. | "The cloud session is complete. I'll check the agent's summary, then follow its link to review the actual changes rather than accepting the result from the summary alone." |
| GitHub Pull Requests view (VS Code) | Review the small diff, focusing on the stylesheet link in `index.html` and the new `styles.css`. Confirm that the routes, form fields, accessibility labels, and `data-testid` attributes are unchanged. | "The diff is small enough to review directly: the template loads a new stylesheet, and the stylesheet handles the layout, form controls, Todo rows, and completed state. I'm also checking that the agent did not change the routes or the hooks used by the existing tests." |
| Pull-request branch + Spring Boot Dashboard | Check out the pull-request branch. Find **springboot-mcp-demo**, select its **Run** action, and wait for the app to show as running on http://localhost:8080. | "The code review shows a focused change; now I need to verify the result. I'll check out the pull-request branch and start the app from the Spring Boot Dashboard." |
| VS Code Simple Browser | Open http://localhost:8080. Show the centered layout and styled controls, then add a Todo called **Review the new design**, complete it, and delete it. Narrow the editor briefly to confirm the page still fits. | "The page now has a clear layout, readable controls, and distinct actions. I'll add a Todo, complete it to check the finished style, and delete it to confirm the original workflow still works. At a narrower width, the layout still fits without changing the app's behavior." |
| GitHub Pull Requests view / Spring Boot Dashboard | Stop the app, return to the pull request, and leave comments if needed. Keep the pull request draft and unmerged during the recording. | "The browser check confirms both the visual improvement and the existing Todo flow. I'll stop the app, return to the pull request, and use the diff and live result to decide whether the change is ready to merge." |

**Capture during recording:** the stylesheet task submitted to the coding agent from a Copilot Chat cloud session.
