...
**Before recording:** Have Java 25, the Extension Pack for Java, and the Spring Boot Extension Pack installed. Open the completed sample in VS Code, stop any running instance of the application, make sure port 8080 is free, make the Maven Explorer and Spring Boot Dashboard views available, and prepare a browser tab for http://localhost:8080. During the take, show each extension pack's **Installed** state; do not reinstall or update extensions.

**Pacing:** Leave Java import, the Maven build, and application startup silent after announcing each wait.

| Where | Do | Say |
|-------|----|-----|
| VS Code — Activity Bar → Extensions | Select the **Extensions** button in the Activity Bar. Search **"Extension Pack for Java"**, open its result, and pause on its **Installed** state. | "I'll open the Extensions view from the Activity Bar and search for the Extension Pack for Java. It is already installed in this setup, giving VS Code Java language support, debugging, Maven, and testing. With the editor tooling confirmed, I'll check which JDK the project uses." |
| VS Code menu → Command Palette | Select **View → Command Palette**, search for **"Java: Configure Java Runtime"**, and select that command from the results. On the Java Runtime page, show Java 25 as the project's JDK, then select the tab's **Close** button. | "The extension pack still needs a JDK, so I'll open the Command Palette from the View menu, search for Java: Configure Java Runtime, and select it from the results. The Java Runtime page shows that this project uses Java 25. I'll close the page, then check the Spring-specific tooling." |
...
| VS Code menu → Command Palette | Select **View → Command Palette**, search for **"Spring Initializr: Create a Maven Project"**, select that command from the results, and pause at the Spring Boot version picker. | "Now I'll open the Command Palette from the View menu, search for Spring Initializr: Create a Maven Project, and select it from the results. The first picker asks for the Spring Boot version, so this is where the project definition begins." |
...
| Dependency picker → folder picker | Add **Spring Web**, **Thymeleaf**, **Actuator**, and **Model Context Protocol Server**. Pause with all four selected, select **Selected 4 dependencies**, then select **Cancel** in the folder picker before a project is generated. | "I'll add Spring Web for HTTP handling and Thymeleaf for the server-rendered page. Actuator adds runtime health information, and Model Context Protocol Server adds MCP server support to the application. These four selections match the completed sample. Because that sample is already open, I'll select Selected 4 dependencies, then cancel at the folder picker before Initializr generates a duplicate project." |
| Explorer | Expand the `com.example.tododemo` package and `templates` folder in the completed sample already open in the workspace. | "The wizard is closed, so I'm back in the completed sample from the repository linked in the description. This sample uses the concise base package `com.example.tododemo` for its Todo code. I'll expand that package and the templates folder to show the Java layers and the web page that were added to the generated starter. I'll start at the class that launches all of them." |
...
| `pom.xml` | Open the root `pom.xml`. Show the Spring Boot parent version `4.1.0`, the `com.example` group and `springboot-mcp-demo` artifact, the Java 25 property, and the four application dependencies selected in Initializr. | "Before I build, I'll open `pom.xml`, Maven's project definition. The Spring Boot parent is version 4.1.0, the group and artifact match the values I entered in Initializr, and the Java version is 25. The dependencies include WebMVC, Thymeleaf, Actuator, and Spring AI's MCP server starter, matching the four capabilities I selected in the wizard. With the project definition connected to the code I just traced, I'll use the Maven view to package the application." |
| Maven Explorer | Expand **springboot-mcp-demo → Lifecycle**, point to **package**, select its **Run** button, and wait for **`BUILD SUCCESS`** in the Maven output. **Do not narrate while Maven builds and runs the tests.** | "I'll open Maven Explorer, expand the project's Lifecycle goals, point to `package`, and select its Run button. Maven will compile the code, run the tests, and create the application Jar. I'll stay silent while it works and continue when the output shows `BUILD SUCCESS`." |
...
**Before recording:** Make sure the five `todo-mcp` tools are disabled in Chat, stop the Spring Boot app and the `todo-mcp` connection, make sure port 8080 is free, and prepare a browser tab at http://localhost:8080. Trust the repository and workspace before the take so only the MCP connection and tool confirmations described below can appear.
...
| Copilot Chat — Agent mode | Enter *"Use the todo-mcp tools to add a todo called 'Email the stakeholders', then list all todos."* and select **Send**. If prompted, review the tool name and arguments before choosing **Allow**. Wait for the final response. | "I'll ask Copilot to use `todo-mcp` to add a Todo called `Email the stakeholders`, then list all Todos, and select Send. If prompted, I'll review the tool name and arguments before allowing it. The final response should contain the newly created item, but I also want to verify it through the application's other interface." |
| Copilot Chat → browser | In Chat, point to **Email the stakeholders** in the structured `list_todos` result. Then select **Refresh** in the prepared http://localhost:8080 browser tab and point to the same title in the Todo list. | "The structured tool result contains `Email the stakeholders`, confirming what the MCP call returned. I'll switch to the prepared browser tab and select Refresh. The same title appears there because the MCP tools and the Thymeleaf controller both delegate to one service and one in-memory repository. That shared result completes the test, so I'll clean up both running connections." |
| `.vscode/mcp.json` → Spring Boot Dashboard | Select **Stop** for `todo-mcp` in `.vscode/mcp.json`. Then select **Stop** for **springboot-mcp-demo** in the Dashboard and confirm it is no longer running. | "I'll return to `.vscode/mcp.json` and select Stop for the `todo-mcp` client connection while its endpoint is still available. Then I'll return to the Spring Boot Dashboard and stop the Java application. Because its repository is in memory, stopping the process clears the Todo created during this demo. Both sides are now closed and port 8080 is free." |
...
> And there it is. I used Spring AI to expose Java operations as tools through the Model Context Protocol, connected those tools to GitHub Copilot in Visual Studio Code, and kept the web interface and Copilot using the same service. After I refresh the browser, a change made through Copilot appears in the web app. What part of your own Java application would you turn into a Copilot tool? Let me know in the comments. Thanks for watching, and happy building.
...
| Where | Do | Say |
|-------|----|-----|
| VS Code — Activity Bar → Extensions | Select the **Extensions** button in the Activity Bar. Search **`@mcp playwright`** and open the Playwright MCP server result. Review its publisher and command configuration. Select **Install**, confirm trust for the server, and wait for its running status. Open the Chat tools picker, show the discovered Playwright tools, disable any that are enabled, and close the picker. | "I've cloned the finished Todo sample from the repository linked in the description and opened it in VS Code with GitHub Copilot signed in. To test the page through a real browser, Copilot needs browser-control tools, so I'll open Extensions from the Activity Bar, search the MCP server gallery for `@mcp playwright`, and open the Playwright result. Before installing it, I'll review the publisher and the command VS Code will run. I'll select Install, confirm that I trust this server, and wait for its status to show that it is running. Finally, I'll open the Chat tools picker and verify that the Playwright tools were discovered. I'll disable any that are already enabled and close the picker, leaving them off while I inspect the page elements they will target." |
| `templates/index.html` | Show `data-testid="new-todo-input"`, `data-testid="add-todo"`, `data-testid="todo-item"`, `th:attr="aria-label='Toggle ' + ${todo.title}"`, and `data-testid="delete-todo"` in the template. | "In `index.html`, the text input has the stable hook `new-todo-input`, and the submit button has `add-todo`. Each rendered row uses `todo-item`, while its checkbox builds an accessible `Toggle` label from that Todo's title. Each delete button uses `delete-todo`. These hooks identify the exact controls without depending on visual styling. With the automation hooks established, I'll first confirm the Java test baseline." |
| VS Code — Activity Bar → Testing | Select the **Testing** button in the Activity Bar, select **Run Tests**, and wait until all Java tests pass. **Do not narrate while the tests run.** | "I'll open the Testing view and select Run Tests. I'll stay silent while the unit and integration tests run, then continue when every test shows as passing. Those tests establish that the service and Spring web flow work below the browser, but they do not exercise this rendered page through a user's controls. For that, the browser tools need a live application." |
...
| Copilot Chat — Agent mode | Enter *"Use the Playwright tools to open http://localhost:8080 and verify the page title is 'Java TODO Demo'. Add a todo called 'Verify the browser flow', find that todo's row, complete it and verify it is checked, then delete it and verify it is gone."* and select **Send**. If prompted, review the tool name and arguments before choosing **Allow**. | "I'll ask Copilot to open the local app and verify its title, add `Verify the browser flow`, find that exact row, complete it and verify its checkbox, then delete it and verify the text is gone. The prompt specifies both the actions and the assertions, so I'll select Send. If prompted, I'll review the tool name and arguments before allowing it. Copilot can then execute the journey while I watch the visible browser state." |
...
| Playwright browser → Spring Boot Dashboard | Select **Close** on the Playwright browser page. Return to the Dashboard, select **Stop** for **springboot-mcp-demo**, and confirm it is no longer running. Leave the Playwright MCP server installed in the VS Code user profile. | "I'll select Close on the Playwright browser page and return to the Spring Boot Dashboard. Then I'll select Stop for `springboot-mcp-demo` and confirm its running indicator clears. The application is shut down, while Playwright remains installed in my VS Code user profile for future Chat sessions." |
