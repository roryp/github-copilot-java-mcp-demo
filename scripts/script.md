...
| Spring Boot Dashboard | Show the app's running status and point to the start, debug, and stop actions. | "The Dashboard finds Spring Boot apps in the workspace, shows their status, and gives me one place to start, debug, or stop them." |
| Editor — `TodoController.java` → browser | Point to the gray URL hints above the controller mappings, then click the root URL. | "While the app is running, Spring Tools adds these gray, clickable URLs to the controller mappings. I'll use the root URL to open the app." |
...
| Spring Boot Dashboard | Use the app's **Debug** action (or `F5`). In the browser add a todo to hit the breakpoint. | "Because I stopped the terminal run, port 8080 is free. The Dashboard can launch the same application with the debugger attached, and submitting a Todo brings execution back to this line." |
